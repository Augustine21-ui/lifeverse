...
export const createGoal = async (req, res) => {
  const userId = req.user.id;
  const { title, description, category, dueDate, milestones } = req.body;
  try {
    const userRes = await query('SELECT course FROM users WHERE id = $1', [userId]);
    const userCourse = userRes.rows[0]?.course || null;
    let milestonesJson = milestones || [];
    // Attach a quiz to each milestone
    milestonesJson = milestonesJson.map(m => ({
      ...m,
      // Ensure is_completed is false by default (if not provided)
      is_completed: m.is_completed || false,
      quiz: generateQuiz(m.title)
    }));
    const result = await query(
      'INSERT INTO goals (user_id, title, description, category, due_date, milestones, target_value, xp_reward, course) VALUES ($1, $2, $3, $4, $5, $6, $7, $8, $9) RETURNING *',
      [userId, title, description, category, dueDate, JSON.stringify(milestonesJson), 100, 50, userCourse]
    );
