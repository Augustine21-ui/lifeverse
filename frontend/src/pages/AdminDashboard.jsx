    ...
    return (
      <div>
        <div className="flex gap-4 mb-4">
          <input type="text" className="input flex-1" placeholder="Search schools..." value={search} onChange={(e) => setSearch(e.target.value)} />
          <button className="btn-primary" onClick={() => { /* Open modal to add subscription */ }}>Add Subscription</button>
        </div>
        <div className="card overflow-x-auto p-0">
          <table className="w-full text-sm">
            <thead className="bg-white/5">
            ...
            <tbody>
              {subscriptions.subscriptions.map(s => (
                <tr key={s.id} className="border-t border-white/5">
                  ...
                  <td className="p-3">{new Date(s.start_date).toLocaleDateString()}</td>
                  <td className="p-3">{s.end_date ? new Date(s.end_date).toLocaleDateString() : '∞'}</td>
                  <td className="p-3">{s.is_active && (s.end_date === null || new Date(s.end_date) > new Date()) ? <CheckCircle size={16} className="text-green-400" /> : <XCircle size={16} className="text-red-400" />}</td>
                  <td className="p-3"><button className="text-white/40 hover:text-brand-400"><Edit size={16} /></button></td>
                </tr>
              ))}
            </tbody>
          </table>
