...
export default function LeaderboardPage() {
  const { user } = useAuth();
  const [leaderboard, setLeaderboard] = useState([]); // Always initialize as array
  ...
  useEffect(() => {
    api.getLeaderboard()
      .then(data => {
        ...
        // 1. Direct array: [ ... ]
        // 2. Object with leaderboard property: { leaderboard: [...] }
  ...
  // Safe guard: if leaderboard is not an array (should never happen), set to empty
  ...
  return (
    <div className="p-6 max-w-2xl mx-auto animate-fade-up">
      ...
      {/* Top 3 podium – only if we have at least 3 entries */}
      {!loading && safeLeaderboard.length >= 3 && (
        <div className="flex items-end justify-center gap-3 mb-8">
          {[safeLeaderboard[1], safeLeaderboard[0], safeLeaderboard[2]].map((entry, i) => {
            if (!entry) return null;
            const rank = [2, 1, 3][i];
            const heights = ['h-20', 'h-28', 'h-16'];
            const initials = (entry.full_name || entry.username || 'U')
              .split(' ')
              .map(w => w[0])
              .join('')
              .slice(0, 2)
            ...
            return (
              <div key={entry.id || i} className="flex flex-col items-center gap-2">
                ...
                <p className="text-xs font-semibold text-center max-w-[70px] truncate">{entry.username || 'User'}</p>
      ...
      {loading ? (
        ...
        <div className="space-y-2">
          {safeLeaderboard.map((entry, idx) => {
            ...
            return (
              <div key={entry.id || idx} className={`card flex items-center gap-4 transition-all duration-150 ${isMe ? 'border border-brand-500/30 bg-brand-500/5' : 'hover:bg-white/[0.06]'}`}>
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2">
                    <span className="font-semibold text-sm truncate">{entry.full_name || entry.username || 'Anonymous'}</span>
                  ...
                  <div className="flex items-center gap-3 text-xs text-white/30 mt-0.5">
                    <span className="flex items-center gap-1"><Flame size={10} className="text-orange-400" />{entry.streak_days || 0}d</span>
                    <span className="flex items-center gap-1"><Award size={10} className="text-violet-400" />{entry.badges_count || 0}</span>
                  </div>
                </div>
          ...
          {safeLeaderboard.length === 0 && (
            <div className="card text-center py-16">
              <p className="text-white/40">No leaderboard data yet. Complete activities to appear!</p>
            </div>
          )}
        </div>
      )}
