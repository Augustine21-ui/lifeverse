...
export default function ParentDashboard() {
  ...
  const loadChildProgress = async (childId) => {
    try {
      const data = await api.getParentChildProgress(childId);
      setProgress(data);
    } catch (err) {
      console.error(err);
    }
  ...
  useEffect(() => {
    if (selectedChild) loadChildProgress(selectedChild.id);
  }, [selectedChild]);
        ...
        <div className="lg:col-span-1">
          <div className="card p-4">
            <h2 className="text-xl font-semibold mb-3 flex items-center gap-2">
              <Users size={20} className="text-brand-400" /> My Children
            </h2>
            {children.length === 0 ? (
              <p className="text-white/40">No children linked yet.</p>
            ) : (
